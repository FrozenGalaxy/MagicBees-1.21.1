...
public final class EffectJarMenu extends AbstractContainerMenu {
    public static final int JAR_SLOT_X = 80, JAR_SLOT_Y = 22;
    ...
    public static EffectJarMenu fromNetwork(int id, Inventory inventory, RegistryFriendlyByteBuf buf) {
        ...
        return new EffectJarMenu(id, inventory, jar, new SimpleContainerData(3));
    }
    ...
    private EffectJarMenu(int id, Inventory inventory, EffectJarBlockEntity jar, ContainerData data) {
        ...
        this.jar = jar;
        this.data = data;
        addSlot(new Slot(jar, 0, JAR_SLOT_X, JAR_SLOT_Y));
        for (int row = 0; row < 3; row++) {
            for (int col = 0; col < 9; col++) addSlot(new Slot(inventory, col + row * 9 + 9, 8 + col * 18, 74 + row * 18));
        }
